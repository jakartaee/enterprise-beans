...
in the case of a Jakarta Messaging Destination, its value might be `javax.jms.Queue`.
...
The environment
naming context introduced in Enterprise Beans 1.1 replaced the Enterprise Beans 1.0 concept of
environment properties.

An Enterprise Beans 1.1 or
later compliant container is not required to implement support for the
Enterprise Beans 1.0 style environment properties. If the container does not
...
If an Enterprise Beans 1.1 or later compliant container
chooses to provide support for the Enterprise Beans 1.0 style environment properties
(so that it can support enterprise beans written to the Enterprise Beans 1.0
specification), it should implement the support as described below.

When the tools convert the Enterprise Beans 1.0 deployment
descriptor to the Enterprise Beans 1.1 XML format, they should place the definitions
...
For example, an Enterprise Beans 1.0 enterprise bean with
two environment properties foo and bar, should declare the following
`env-entry` elements in its Enterprise Beans 1.1 format deployment descriptor.
...
The enterprise bean uses the Enterprise Beans 1.0 API to
