...
factory.footnote:a10232[Except for the `jakarta.mail.Session` resource 
...
`jakarta.transaction.UserTransaction` interface. The instance can use this
...
any cached updates to the database.footnote:a10242[Note that if the Jakarta 
Persistence is used, the persistence provider will use the 
...
Java RMI if there is a `create<METHOD>` corresponding to the
...
method must be legal types for Java RMI if the method corresponds to a
...
types for Java RMI. The remote business interface is not required or
...
follow the rules for Java RMI. This means that their argument and return
values must be of valid types for Java RMI, and their `throws` clauses
...
Java RMI rules for the definition of remote interfaces.
...
* The methods defined in this interface must
follow the rules for Java RMI. This means that their argument and return
values must be of valid types for Java RMI, and that their `throws`
clauses must include the `java.rmi.RemoteException`.

* The remote home interface is allowed to have
superinterfaces. Use of interface inheritance is subject to the Java RMI
