...
bean’s{empty}footnote:a10230[] local component interface, even if it is
not serializable.

A reference to an entity
bean’s{empty}footnote:a10231[Component contract and client view of entity 
...
methods{empty}footnote:a10246[If a client calls lifecycle callback method 
...
callback interceptor methods{empty}footnote:a10250[If a client calls lifecycle 
...
PostConstruct, PreDestroy lifecycle
callback interceptor methods{empty}footnote:a10252[If a client calls lifecycle 
callback method through a business interface or a no-interface view, 
the method is treated like a business method.]
