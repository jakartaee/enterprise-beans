...
persistent conversational state. See the document _"Java Persistence
API"_ specification <<a9851>>.
...
the behavior is undefined by the EJB architecture.
...
`javax.ejb.NoSuchEJBException` exception as defined by <<a304>> 
and <<a312>>.
...
may implement the `javax.ejb.TimedObject` interface. See <<a5456>>.
...
the `javax.ejb.EnterpriseBean` interfaces._
...
_Compatibility Note: EJB 1.0 allowed the `ejbCreate` method to
...
non-application exceptions to the container (see <<a2986>>). An EJB 2.0 and
...
_Compatibility Note: EJB 1.0 allowed the business methods to throw
...
container (see <<a2986>>). An EJB 2.0 or later compliant enterprise bean 
...
defined by the `javax.ejb` package are excluded when determining whether
