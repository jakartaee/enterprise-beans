...
bean’sfootnote:a10230[] local component interface, even if it is
not serializable.

A reference to an entity
bean’sfootnote:a10231[Component contract and client view of entity 
...
session beans and must not be usedfootnote:a10237[The concurrency 
...
_javax.ejb.ConcurrentAccessException_footnote:a10238[The 
...
callback methodfootnote:a10241[If a stateful session bean lifecycle 
...
_beforeCompletion_ callback methodfootnote:a10241[]. When _beforeCompletion_ 
is invoked, the instance should write
any cached updates to the databasefootnote:a10242[Note that if the Java 
...
_afterCompletion_ callback methodfootnote:a10241[], specifying the status of the completion (either commit or
...
_javax.ejb.NoSuchEJBException_footnote:a10243[If the business interface 
...
_javax.ejb.NoSuchEJBException_footnote:a10245[If the business interface 
...
methodsfootnote:a10246[If a client calls lifecycle callback method 
...
_javax.ejb.NoSuchEJBException_footnote:a10247[If the business interface is 
...
of the bean’s business interfacefootnote:a10248[If the business interface 
is a remote business interface that extends `java.rmi.Remote`, the 
...
create a new session bean instancefootnote:a10249[If an `AroundConstruct` 
...
callback interceptor methodsfootnote:a10250[If a client calls lifecycle 
...
starts when the container invokes the session bean class constructor to
create the singleton bean instancefootnote:a10251[If an `AroundConstruct` 
...
{empty}PostConstruct, PreDestroy lifecycle
callback interceptor methodsfootnote:a10252[If a client calls lifecycle 
callback method through a business interface or a no-interface view, 
the method is treated like a business method.]
...
filesfootnote:a10253[Note that the interfaces provided by the Bean 
