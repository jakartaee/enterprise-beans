...
the invoked method has the _REQUIRED_footnote:a10264[In this chapter we 
...
Enterprise JavaBeansfootnote:a10266[This restriction may be removed 
...
_method3._ footnote:a10268[Note that the Bean Provider must use the 
...
coordination of the transaction commitfootnote:a10272[The container 
...
Throw the
_javax.ejb.EJBException_footnote:a10274[If the business interface is a 
...
_javax.ejb.EJBTransactionRequiredException_footnote:a10275[If the business 
...
If the client calls with a transaction
context, the container throws the
_javax.ejb.EJBException_footnote:a10276[If the business interface is a 
remote business interface that extends `java.rmi.Remote`, the 
`java.rmi.RemoteException` is thrown to the client instead.].
If the EJB 2.1 client
...
An entity
objectfootnote:a10279[See <<a1702>>.] may be accessed by multiple 
clients in the
...
the entity object within the same transactionfootnote:a10281[This diamond 
...
objectfootnote:a10280[Component contract and client view of entity beans 
...
case involving an entity objectfootnote:a10280[Component contract and 
