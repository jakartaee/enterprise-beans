...
_The EJB architecture does not specify how an
...
_The EJB specification does not define the
...
_The case of multiple domains on the same EJB
...
_The typical security functions provided by the EJB server include:_

* _Authentication of principals._

* _Access authorization for EJB calls and resource manager access._
...
The EJB Container Provider is responsible for providing secure access to 
...
The EJB specification does not define the "system" principal under which 
...
`javax.ejb.EJBAccessException`.footnote:a10330[If the business interface 
...
client, or the `javax.ejb.EJBException` (or
its subclass, the `javax.ejb.AccessLocalException`) if the client is a
local client.
