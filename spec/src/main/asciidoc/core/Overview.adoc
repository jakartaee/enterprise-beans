...
To help
interoperability for Enterprise Beans environments that include systems from multiple
vendors, the Enterprise Beans specification requires compliant implementations to
support the interoperability protocol based on CORBA/IIOP for remote
invocations from Jakarta EE clients. Implementations may support other
...
Jakarta(R) XML RPC <<a9873>> is optional
as of the 3.2 release of the Enterprise Beans specification. See the
Enterprise Beans Optional Features document <<a9890>> for the complete
description.
