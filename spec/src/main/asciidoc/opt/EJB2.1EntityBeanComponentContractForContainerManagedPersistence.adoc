...
+
This method executes with an unspecified transaction context (Refer to EJB Core Contracts and Requirements document <<a3339>> Subsection "`Handling
...
Such an entity bean does not allow its clients to create new EJB objects.
...
+
The `ejbHome<METHOD>` method executes in the transaction context determined by the transaction attribute of the matching `<METHOD>` home method, as described in EJB Core Contracts and
